  ...
  const resendOtp = () => {
    ...
    if (!email) {
      context.setAlertBox({
        open: true,
        error: true,
        msg: "Email is missing. Please sign in again.",
      });
      history("/signIn");
      return;
    }
    ...
    setIsLoading(true);
    postData("/api/user/verifyAccount/resendOtp", { email }).then((res) => {
      context.setAlertBox({
        open: true,
        error: res?.success !== true,
        msg: res?.message || res?.msg || "Unable to send OTP",
      });
      setIsLoading(false);
    });
  };
        ...
        <div className="box card p-3 shadow border-0">
          ...
          <form className="mt-3" onSubmit={verify}>
            ...
            <p className="text-center">
              <a className="border-effect cursor txt" onClick={resendOtp}>
                Resend OTP
              </a>
            </p>
          </form>
